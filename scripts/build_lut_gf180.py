#!/usr/bin/env python3
"""
GF180 gm/ID lookup table + plots, generalized from the sky130 pfet script to
handle either nfet_03v3 or pfet_03v3, at VDD=3.3V.

Same fix as before: sort by the true swept variable (VGS/VSG), not by the
noisy derived gm_id, and lightly smooth ID before differentiating.

Usage:
    cd ~/gf180_gmid_sweep_output
    python3 build_lut_gf180.py nfet
    python3 build_lut_gf180.py pfet
"""
...
if len(sys.argv) != 2 or sys.argv[1] not in ("nfet", "pfet"):
    print("Usage: python3 build_lut_gf180.py nfet|pfet")
    sys.exit(1)

DEVICE = sys.argv[1]           # "nfet" or "pfet"
W = 1.0e-6                     # must match the W you used in gf180_gmid_sweep.sh
VDD = 3.3                      # GF180 03v3 device supply
DATA_GLOB = f"{DEVICE}_L*_VD*.txt"
LUT_OUT = f"{DEVICE}_03v3_gmid_lut.csv"
ID_FLOOR = 1e-9
SMOOTH_WINDOW = 7
# ---------------------------------------------

FNAME_RE = re.compile(rf"{DEVICE}_L([\dp]+)_VD([\dp]+)\.txt")
...
def gate_to_x(vg, device):
    """VGS for nfet (source grounded, no transform); VSG for pfet (source at VDD)."""
    if device == "nfet":
        return vg
    else:
        return VDD - vg
...
def main():
    ...
    for L, vd_map in sorted(by_L.items()):
        ...
        if ok and len(id_cols) >= 2:
            ...
            if DEVICE == "nfet":
                # VD is already VDS directly -- no sign flip needed.
                gds_2d = np.gradient(id_2d, vd_vals, axis=1)
            else:
                # Same as the original pfet script: VD is absolute node volts,
                # source sits at VDD, so VSD = VDD - vd_val -- flip the sign.
                gds_2d = -np.gradient(id_2d, vd_vals, axis=1)
    ...
    ax.set_title(f"{DEVICE}_03v3: {xlabel}, ID/W, gm/gds vs gm/ID (VD_abs={vd_for_plot} V)")
    ax.grid(True)
    ax.legend()
